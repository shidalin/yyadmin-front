...
<div>
    ...
    <!--数据表格  -->
    <Table :data="organizations" :columns="organizationTableColumns" height="500" @on-selection-change="selectionChange" stripe border ref="organizationDataTable"></Table>
    <!--分页  -->
    ...
    <div>
        <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="1200" title="组织信息" ok-text="保存" @on-cancel="modalClosedEvent" @on-ok="modalConfirmEvent">
            <Form label-position="left" :label-width="100" ref="modalForm" :model="modalForm" :rules="modalRule">
                ...
                <Row>
                    <Col span="11">
                    <Form-item label="上级组织" prop="orgPid">
...
export default {
    ...
    created() {
        let self = this;
        self.list_organization(self.current, self.pageSize);
    },
    methods: {
        btnAdd() {
            //数据新增
        ...
        btnRemove(params) {
            ...
            //远程持久化数据
            self.$http.post("/system/organization/remove/" + params.row.id).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnListRemove() {
            ...
            //远程持久化数据
            self.$http.post("/system/organization/remove", ids).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnUpdate(params) {
            ...
            self.modalIndex = params.index;
            //远程加载数据
            self.$http.post("/system/organization/detail/" + id).then(response => {
        ...
        list_organization(current, pageSize) {
            ...
            self.$http
                ...
                .then(response => {
                    self.organizations = response.data.data.records;
        ...
        btnQuery() {
            ...
            if (self.queryField !== "" && self.queryValue !== "") {
                ...
                self.$http.post("/system/organization/list", {
                    current: self.current,
                    size: self.pageSize,
                    //构造查询条件
                    condition: condition
        ...
        btnPrint() {
            let self = this;
            let table = this.$refs.organizationDataTable.$el;
        ...
        modalConfirmEvent() {
            ...
            if (
                ...
                // 远程持久化数据-新增
                self.$http.post("/system/organization/add", self.modalForm).then(response => {
                    //刷新数据，跳转到最后一页
                ...
                //远程持久化数据-更新
                self.$http.post("/system/organization/update", self.modalForm).then(result => {
                    //局部更新数据
                    self.pageRefreshEvent(self.current, self.pageSize);
