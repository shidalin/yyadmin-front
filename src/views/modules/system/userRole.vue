...
<div>
    ...
    <!--数据表格  -->
    <Table :data="userRoles" :columns="userRoleTableColumns" height="500" stripe></Table>
    <!--分页  -->
    ...
    <!--明细信息  -->
    <div>
        <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="800" title="用户-角色信息" ok-text="保存" @on-cancel="modalClosedEvent" @on-ok="modalConfirmEvent">
            <Form label-position="left" :label-width="80" ref="modalForm" :model="modalForm" :rules="modalRule">
                <Row>
                    <Col span="11">
                    <Form-item label="" prop="id">
                        <Input v-model="modalForm.id" type="text" placeholder="请输入"></Input>
                    </Form-item>
                    </Col>
                ...
                <Row>
                    ...
                    <Col span="11" offset="2">
                    <Form-item label="用户-角色状态" prop="status">
                        <Input v-model="modalForm.status" type="text" placeholder="请输入用户-角色状态"></Input>
                    </Form-item>
                    </Col>
                </Row>
                <Row>
                    ...
                    <Form-item label="用户主键" prop="userId">
                        <Input v-model="modalForm.userId" type="text" placeholder="请输入用户主键"></Input>
                    </Form-item>
                    </Col>
                    <Col span="11" offset="2">
...
export default {
    ...
    methods: {
        ...
        list_userRole(current, pageSize) {
            ...
            promise.then((result) => {
                this.userRoles = result.data.list;
        ...
        modalConfirmEvent() {
            ...
            if (self.modalForm.id == undefined || self.modalForm.id == null || self.modalForm.id == "") {
                ...
                promise.then((result) => {
                    //局部更新数据
                    Vue.set(self.userRoles, self.modalIndex, result.data.data);
                    self.$Message.success("保存成功");
                })
            }
        },
        pageRefreshEvent(current, pageSize) {
            this.list_userRole(current, pageSize);
        }
