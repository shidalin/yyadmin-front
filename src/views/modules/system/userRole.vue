...
<div>
    ...
    <!--数据表格  -->
    <Table :data="userRoles" :columns="userRoleTableColumns" height="500" @on-selection-change="selectionChange" stripe border ref="userRoleDataTable"></Table>
    <!--分页  -->
    ...
    <!--明细信息  -->
    <div>
        <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="1200" title="用户角色关联表信息" ok-text="保存" @on-cancel="modalClosedEvent" @on-ok="modalConfirmEvent">
            <Form label-position="left" :label-width="100" ref="modalForm" :model="modalForm" :rules="modalRule">
                <Row>
                    <Col span="11">
                    <Form-item label="关联用户" prop="userId">
                        <Input v-model="modalForm.userId" type="text" placeholder="请输入关联用户"></Input>
                    </Form-item>
                    </Col>
                    <Col span="1"> &nbsp;
                    </Col>
...
export default {
    data() {
        ...
        return {
            ...
            pageSize: 10,
            //table表格
            userRoleTableColumns: tableColumns,
            //父子通信属性
            modalShow: false,
            modalForm: {},
            modalRule: {},
    ...
    created() {
        let self = this;
        self.list_userRole(self.current, self.pageSize);
    },
    methods: {
        btnAdd() {
            //数据新增
        ...
        btnRemove(params) {
            ...
            //远程持久化数据
            self.$http.post("/system/userRole/remove/" + params.row.id).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnListRemove() {
            ...
            //远程持久化数据
            self.$http.post("/system/userRole/remove", ids).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnUpdate(params) {
            ...
            self.modalIndex = params.index;
            //远程加载数据
            self.$http.post("/system/userRole/detail/" + id).then(response => {
                self.modalForm = response.data.data;
            });
        },
        //远程请求数据
        list_userRole(current, pageSize) {
            //分页查询
            let self = this;
            //远程请求数据
            self.$http
                .post("/system/userRole/list", {
                    current: self.current,
                    size: self.pageSize
                })
                .then(response => {
                    self.userRoles = response.data.data.records;
        ...
        btnQuery() {
            ...
            if (self.queryField !== "" && self.queryValue !== "") {
                ...
                self.$http.post("/system/userRole/list", {
                    current: self.current,
                    size: self.pageSize,
                    //构造查询条件
                    condition: condition
        ...
        btnPrint() {
            let self = this;
            let table = this.$refs.userRoleDataTable.$el;
        ...
        modalConfirmEvent() {
            ...
            if (
                ...
                // 远程持久化数据-新增
                self.$http.post("/system/userRole/add", self.modalForm).then(response => {
                    //刷新数据，跳转到最后一页
                ...
                //远程持久化数据-更新
                self.$http.post("/system/userRole/update", self.modalForm).then(result => {
                    //局部更新数据
                    self.pageRefreshEvent(self.current, self.pageSize);
        ...
        changePage(current) {
            ...
            self.list_userRole(self.current, self.pageSize);
            this.list_userRole(this.current, this.pageSize);
        },
        //切换每页条数
