...
<div>
    ...
    <!--数据表格  -->
    <Table :data="menus" :columns="menuTableColumns" height="500" @on-selection-change="selectionChange" stripe border ref="menuDataTable"></Table>
    <!--分页  -->
...
export default {
    ...
    created() {
        let self = this;
        self.list_menu(self.current, self.pageSize);
    },
    methods: {
        btnAdd() {
            //数据新增
        ...
        btnRemove(params) {
            ...
            //远程持久化数据
            self.$http.post("/system/menu/remove/" + params.row.id).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnListRemove() {
            ...
            //远程持久化数据
            self.$http.post("/system/menu/remove", ids).then(result => {
                //计算当前页数
                //向下取整函数 Math.trunc()
                //删除页数
        ...
        btnUpdate(params) {
            ...
            self.modalIndex = params.index;
            //远程加载数据
            self.$http.post("/system/menu/detail/" + id).then(response => {
        ...
        list_menu(current, pageSize) {
            ...
            self.$http
                ...
                .then(response => {
                    self.menus = response.data.data.records;
        ...
        btnQuery() {
            ...
            if (self.queryField !== "" && self.queryValue !== "") {
                ...
                self.$http.post("/system/menu/list", {
                    current: self.current,
                    size: self.pageSize,
                    //构造查询条件
                    condition: condition
        ...
        btnPrint() {
            let self = this;
            let table = this.$refs.menuDataTable.$el;
        ...
        modalConfirmEvent() {
            ...
            if (
                ...
                // 远程持久化数据-新增
                self.$http.post("/system/menu/add", self.modalForm).then(response => {
                    //刷新数据，跳转到最后一页
                ...
                //远程持久化数据-更新
                self.$http.post("/system/menu/update", self.modalForm).then(result => {
                    //局部更新数据
                    self.pageRefreshEvent(self.current, self.pageSize);
