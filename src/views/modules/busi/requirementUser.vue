<template>
    <div>
        ...
        <!--数据表格  -->
        <Table :data="requirementUsers" :columns="requirementUserTableColumns" height="500" stripe></Table>
        <!--分页  -->
        ...
        <!--明细信息  -->
        <div>
            <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="800" title="需求-用户信息" ok-text="保存"
                ...
                <Form label-position="left" :label-width="80" ref="modalForm" :model="modalForm" :rules="modalRule">
                        ...
                        <Row>
                            ...
                            <Form-item label="最后修改时间" prop="gmtModified">
                                ...
                                <Col span="11" offset="2">
                                <Form-item label="需求主键"
                                           prop="pkRequirement">
                                    <Input v-model="modalForm.pkRequirement" type="text"
                        ...
                        <Row>
                            ...
                            <Form-item label="人员主键" prop="pkUser">
                                <Input v-model="modalForm.pkUser" type="text"
                                       placeholder="请输入人员主键"></Input>
                            </Form-item>
                            </Col>
                                <Col span="11" offset="2">
                                <Form-item label="人员类别"
                                           prop="requirementUserType">
                                    <Input v-model="modalForm.requirementUserType" type="text"
                                           placeholder="请输入人员类别"></Input>
                                </Form-item>
                                </Col>
                        </Row>
                </Form>
    ...
    export default {
        ...
        ,
            ...
            return {
                ...
                pageSize: 10,
                //table表格
                requirementUserTableColumns: tableColumns,
                //父子通信属性
                modalShow: false,
                modalForm: {},
                modalRule: {},
        ...
        //钩子方法，页面渲染结束后加载
        created() {
            this.list_requirementUser(this.current, this.pageSize);
        },
        methods: {
            //远程请求数据
            list_requirementUser(current, pageSize) {
                //参数
                let params = {
                    'page': current,
                    'size': pageSize
                ...
                ;
                promise.then((result) => {
                    this.requirementUsers = result.data.list;
            ...
            modalConfirmEvent() {
                ...
                if (self.modalForm.id == undefined || self.modalForm.id == null || self.modalForm.id == "") {
                    ...
                    promise.then((result) => {
                        //局部更新数据
                        Vue.set(self.requirementUsers, self.modalIndex, result.data.data);
                    self.$Message.success("保存成功");
                })
                }
            },
            pageRefreshEvent(current, pageSize) {
                this.list_requirementUser(current, pageSize);
            }
