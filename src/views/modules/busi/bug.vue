<template>
    <div>
        ...
        <!--数据表格  -->
        <Table :data="bugs" :columns="bugTableColumns" height="500" stripe></Table>
        <!--分页  -->
        ...
        <!--明细信息  -->
        <div>
            <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="800" title="BUG信息信息" ok-text="保存" @on-cancel="modalClosedEvent" @on-ok="modalConfirmEvent">
                <Form label-position="left" :label-width="80" ref="modalForm" :model="modalForm" :rules="modalRule">
                    <Row>
                        <Col span="11">
                        <Form-item label="BUG主键" prop="id">
                            <Input v-model="modalForm.id" type="text" placeholder="请输入BUG主键"></Input>
                        </Form-item>
                        </Col>
                    ...
                    <Row>
                        ...
                        <Col span="11" offset="2">
                        <Form-item label="BUG编码" prop="bugCode">
                            <Input v-model="modalForm.bugCode" type="text" placeholder="请输入BUG编码"></Input>
                        </Form-item>
                        </Col>
                    </Row>
                    <Row>
                        <Col span="11">
                        <Form-item label="BUG名称" prop="bugName">
                            <Input v-model="modalForm.bugName" type="text" placeholder="请输入BUG名称"></Input>
                        </Form-item>
                        </Col>
                        <Col span="11" offset="2">
                        <Form-item label="BUG内容" prop="bugContent">
                            <Input v-model="modalForm.bugContent" type="text" placeholder="请输入BUG内容"></Input>
                        </Form-item>
                        </Col>
                    </Row>
                    <Row>
...
export default {
    ...
    //钩子方法，页面渲染结束后加载
    created() {
        this.list_bug(this.current, this.pageSize);
    },
    methods: {
        //远程请求数据
        list_bug(current, pageSize) {
            //参数
            let params = {
                'page': current,
                'size': pageSize
        ...
        modalConfirmEvent() {
            ...
            if (self.modalForm.id == undefined || self.modalForm.id == null || self.modalForm.id == "") {
                ...
                promise.then((result) => {
                    //局部更新数据
                    Vue.set(self.bugs, self.modalIndex, result.data.data);
                    self.$Message.success("保存成功");
                })
            }
        },
        pageRefreshEvent(current, pageSize) {
            this.list_bug(current, pageSize);
        }
