<template>
    <div>
        ...
        <!--数据表格  -->
        <Table :data="projectUsers" :columns="projectUserTableColumns" height="500" stripe></Table>
        <!--分页  -->
        ...
        <!--明细信息  -->
        <div>
            <Modal :mask-closable="false" :closable="false" v-model="modalShow" width="800" title="项目-用户信息" ok-text="保存"
                ...
                <Form label-position="left" :label-width="80" ref="modalForm" :model="modalForm" :rules="modalRule">
                        <Row>
                            <Col span="11">
                            <Form-item label="项目-人员主键" prop="id">
                                <Input v-model="modalForm.id" type="text"
                                       placeholder="请输入项目-人员主键"></Input>
                            </Form-item>
                            </Col>
                        ...
                        <Row>
                            <Col span="11">
                            <Form-item label="项目成员" prop="pkUser">
                                <Input v-model="modalForm.pkUser" type="text"
                                       placeholder="请输入项目成员"></Input>
                            </Form-item>
                            </Col>
                        </Row>
                </Form>
    ...
    export default {
        ...
        ,
            ...
            return {
                ...
                pageSize: 10,
                //table表格
                projectUserTableColumns: tableColumns,
                //父子通信属性
                modalShow: false,
                modalForm: {},
                modalRule: {},
        ...
        //钩子方法，页面渲染结束后加载
        created() {
            this.list_projectUser(this.current, this.pageSize);
        },
        methods: {
            //远程请求数据
            list_projectUser(current, pageSize) {
                //参数
                let params = {
                    'page': current,
                    'size': pageSize
                ...
                ;
                promise.then((result) => {
                    this.projectUsers = result.data.list;
            ...
            modalConfirmEvent() {
                ...
                if (self.modalForm.id == undefined || self.modalForm.id == null || self.modalForm.id == "") {
                    ...
                    promise.then((result) => {
                        //局部更新数据
                        Vue.set(self.projectUsers, self.modalIndex, result.data.data);
                    self.$Message.success("保存成功");
                })
                }
            },
            pageRefreshEvent(current, pageSize) {
                this.list_projectUser(current, pageSize);
            }
